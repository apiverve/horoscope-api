...
class HoroscopeValidationException implements Exception {
  final List<String> errors;

  HoroscopeValidationException(this.errors);

  @override
  String toString() => 'HoroscopeValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://horoscope.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [sign] (required) - The zodiac sign for which you want to get the horoscope
/// * [yesterday] - Set to true to get yesterday's horoscope
/// * [tomorrow] - Set to true to get tomorrow's horoscope
class HoroscopeClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'sign': ValidationRule(type: 'string', required: true),
    'yesterday': ValidationRule(type: 'boolean', required: false),
    'tomorrow': ValidationRule(type: 'boolean', required: false),
  };

  HoroscopeClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/horoscope',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [HoroscopeValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw HoroscopeValidationException(errors);
    }
  }

  /// Execute a request to the Horoscope API.
  ///
  /// Parameters are validated before sending the request.
  Future<HoroscopeResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw HoroscopeException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return HoroscopeResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw HoroscopeException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw HoroscopeException('Resource not found');
      } else {
        throw HoroscopeException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is HoroscopeException) rethrow;
      throw HoroscopeException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Horoscope API client.
class HoroscopeException implements Exception {
  ...
  @override
  String toString() => 'HoroscopeException: $message';
}
