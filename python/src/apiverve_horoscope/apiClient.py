...
class HoroscopeAPIClientError(Exception):
...
class ValidationError(HoroscopeAPIClientError):
...
class HoroscopeAPIClient:
    ...
    VALIDATION_RULES = {"sign": {"type": "string", "required": True}, "yesterday": {"type": "boolean", "required": False, "default": False}, "tomorrow": {"type": "boolean", "required": False, "default": False}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the HoroscopeAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises HoroscopeAPIClientError: If API key is invalid
        """
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[HoroscopeAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns HoroscopeData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises HoroscopeAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise HoroscopeAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise HoroscopeAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise HoroscopeAPIClientError(f"Request failed: {str(e)}")
